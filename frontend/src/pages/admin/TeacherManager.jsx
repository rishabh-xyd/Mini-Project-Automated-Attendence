...
import { UserPlus, Pencil, Trash2, Save, X, Briefcase, Camera, Upload } from 'lucide-react';
...
export default function TeacherManager() {
    ...
    const fetchTeachers = async () => {
        try {
            const res = await axios.get('http://localhost:8000/admin/users?role=teacher');
    ...
    const handleUpload = async () => {
        ...
        try {
            ...
            setTimeout(() => {
                closeUploadModal();
                fetchTeachers(); // Refresh to potentially show upload status if we tracked it
            }, 1000);
        } catch (err) {
    ...
    return (
        <div className="p-6 bg-slate-900 rounded-xl border border-slate-800 shadow-xl relative">
            ...
            <div className="mb-8 bg-slate-950 p-6 rounded-xl border border-slate-800">
                <h3 className="text-lg font-semibold text-slate-200 mb-4">{editingId ? 'Edit Faculty' : 'Register New Faculty'}</h3>
                ...
                <form onSubmit={editingId ? handleUpdate : handleCreate} className="space-y-4">
                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
                        ...
                        <div>
                            ...
                            <input value={employeeId} onChange={e => setEmployeeId(e.target.value)} className="w-full bg-slate-900 border border-slate-700 rounded p-2 text-white text-sm" required />
                        </div>
                        <div>
                            <label className="block text-xs text-slate-400 mb-1">Department</label>
                            <input value={department} onChange={e => setDepartment(e.target.value)} className="w-full bg-slate-900 border border-slate-700 rounded p-2 text-white text-sm" placeholder="CS, IT..." required />
                        </div>
                        <div>
                                    ...
                                    {uploadMode === 'camera' && (
                                        <div className="flex flex-col items-center gap-3">
                                            ...
                                            <div className="flex gap-2">
                                                {!stream ? (
                                                    <button type='button' onClick={startCamera} className="bg-purple-600 hover:bg-purple-700 text-white px-4 py-2 rounded-lg text-xs font-medium flex items-center gap-2 transition-colors">
                    ...
                    <div className="flex gap-2 pt-4">
                        <button type="submit" disabled={!editingId && !selectedFile} className="flex-1 bg-purple-600 hover:bg-purple-700 disabled:opacity-50 disabled:cursor-not-allowed text-white p-2 rounded text-sm font-medium transition-colors flex items-center justify-center gap-2">
                            {editingId ? <Save className="w-4 h-4" /> : <UserPlus className="w-4 h-4" />}
                            {editingId ? 'Update' : 'Register Faculty & Face'}
                        </button>
                        {editingId && (
                ...
                <table className="w-full text-left text-sm text-slate-400">
                    ...
                    <tbody className="divide-y divide-slate-800">
                        {loading ? <tr><td colSpan="3" className="p-4 text-center">Loading...</td></tr> :
                            teachers.length === 0 ? <tr><td colSpan="3" className="p-4 text-center">No faculty found.</td></tr> :
                                teachers.map(t => (
                                    <tr key={t.id} className="hover:bg-slate-800/50">
                                        <td className="px-4 py-3 font-mono text-slate-300">{t.employee_id || '-'}</td>
                                        <td className="px-4 py-3 font-medium text-white">{t.name}</td>
                                        <td className="px-4 py-3">{t.department || '-'}</td>
                                        <td className="px-4 py-3">{t.email}</td>
                                        <td className="px-4 py-3 text-right flex justify-end gap-2">
                                            <button onClick={() => openUploadModal(t)} className="p-1 hover:text-green-400 transition-colors" title="Upload Face"><Camera className="w-4 h-4" /></button>
                                            <button onClick={() => startEdit(t)} className="p-1 hover:text-blue-400 transition-colors"><Pencil className="w-4 h-4" /></button>
                                            <button onClick={() => handleDelete(t.id)} className="p-1 hover:text-red-400 transition-colors"><Trash2 className="w-4 h-4" /></button>
                                        </td>
                                    </tr>
                                ))}
                    </tbody>
                ...
                <div className="fixed inset-0 bg-black/80 backdrop-blur-sm flex items-center justify-center z-50">
                    <div className="bg-slate-900 border border-slate-700 p-6 rounded-2xl w-full max-w-md shadow-2xl">
                        ...
                        <div className="space-y-4">
                            ...
                            <button onClick={handleUpload} disabled={!selectedFile} className="w-full py-2 bg-purple-600 hover:bg-purple-700 disabled:opacity-50 disabled:cursor-not-allowed text-white rounded-lg font-medium transition-colors">
